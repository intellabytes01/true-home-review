...
class EditJob extends Component {
    ...
    getJobDetails = (form) => {
        ...
        _callApi(form,
            ...
            if (error) {
                ...
                let { jobDetail, addressLatLng } = this.state


                jobDetail = { ...{}, ...result }
                let inspector = jobDetail.inspector;
                // this.getDateFormat(result.report.scheduledTime)
                jobDetail.report.scheduledTime = this.getDateFormat(result.report.scheduledTime)
                jobDetail.inspector = { value: inspector.userID, text: inspector.firstName + " " + inspector.lastName }

                // if (!jobDetail['builders'] &&  ) {

                //     jobDetail['builders'] = []
                // }
                                            

                this.handleDepDropdown(jobDetail.report.clientID)
                ...
                setTimeout(() => {
                    ...
                    if (this.state.changeLocation) {
                        ...
                        let temp = { lat: +result.report.latitude, lng: +result.report.longitude }
                        this.state.changeLocation(temp);

                        addressLatLng.longitude = temp.lng
                        addressLatLng.latitude = temp.lat
                    }
                    this.setState(() => ({ jobDetail, addressLatLng }))
                }, 100)
            }
        })
    ...
    onMapMounted = (ref, changeLocation) => {
        const { maps } = window.google;
        let geoCoder = new maps.Geocoder()


        this.setState(() => ({ maps, geoCoder, changeLocation }))

        let reportID = localStorage.getItem("edit_in_re_po_id")
    ...
    handleNewJob = (values, formikBag) => {
        ...
        let { addressLatLng, jobEditResponse } = this.state
        ...
        _callApi(
            ...
            if (error) {
                ...
                jobEditResponse.isSuccess = false
                jobEditResponse.message = error.message
            ...
            this.setState(()=>({jobEditResponse}))

            this.clearJobEditResponse()
            
        }).catch(()=>{

            jobEditResponse.isSuccess = false
            jobEditResponse.message = "Something went wrong"
    ...
    clearJobEditResponse = ()=>{

        let { jobEditResponse } = this.state
        jobEditResponse.isSuccess = false
        jobEditResponse.message = ""
    ...
    removeOldImages = (index) => {


        if (this.state.jobDetail && this.state.jobDetail['floorplan']) {

            let { jobDetail } = this.state

            jobDetail.floorplan.splice(index, 1)

            // jobDetail['floorplan'] =floorplan
    ...
    getLatLng = (params) => {
        ...
        if (params) {
            ...
            return { lng: params.longitude, lat: params.latitude }
        }
    }
    uploadImages = (file) => {
            ...
            <div className="add-new-job popup-wrapper">
                <div className="add-new-job-inner pop-form-wrapper">
                    <div className="popup-header text-center pt-3">
                        <h2> {this.state.toggleForm && !this.state.reportID ? "Edit Job" : "Upload Images"} </h2>
                    </div>
                    <div className="popup-body p-4">
                        ...
                        <Formik
                            initialValues={
                                (this.state.jobDetail) ? { ...this.state.jobDetail } :
                                    {
                                        report: {
                                            userID: null,
                                            clientID: null,
                            ...
                            render={({ values, errors, handleSubmit, handleChange, handleBlur, setFieldValue, setFieldError, setFieldTouched, touched, submitCount, isSubmitting }) => (
                                <form onSubmit={handleSubmit} >
                                    ...
                                    <span className="d-flex justify-content-center" style={{ color: "red" }} > {!this.state.jobEditResponse.isSuccess  ? this.state.jobEditResponse.message  : null}</span>
                                    <div className="row">
                                                ...
                                                <div className="row">
                                                    <div className="col-md-6 pl-2 pr-2">
                                                        <SearchAbleSelect
                                                            ...
                                                            // END
                                                            disabled={this.state.jobDetail && this.state.jobDetail['report'] && this.state.jobDetail['report']["clientID"] && +this.state.jobDetail['report']["clientID"] !== 0   ? true : false }
                            ...
                            <span  className="d-block add-more-title">Add Builder</span> */}
                                                ...
                                                <div className="col-md-12">
                                                    <div className="map-card">
                                                        <GoogleMapComponent
                                                            ...
                                                            center={(this.state.jobDetail) ? this.getLatLng(this.state.jobDetail['report']) : null}
                                    ...
                                    <div className="pop-form-wrapper text-center mt-4">
                                        <ButtonPrimary btntext="Save" disabled={isSubmitting} type="submit" className="button-primary" />
                                    </div>
                                </form>
                            )}
                        ...
                        <div className="row mt-4">
                            ...
                            {/* <div className="clearfix">
                                {this.state.jobDetail && this.state.jobDetail['floorplan'] && this.state.jobDetail['floorplan'].map((item, index) => (
                                    <div key={item.photo} className="dropzone-image">
                                        <img
                                            alt="Preview"
                                            key={item.photo}
                                            src={item.photo}
                                        ...
                                        <span
                                            className="cross-btn"
                                            onClick={(event) => { event.stopPropagation(); this.removeOldImages(index) }}
                                            title="sdfgh">
                                                            ...
                                                            <React.Fragment>
                                                                <div className="clearfix">
                                                                    {this.state.files.map((file, index) => (
                                                                        <div key={file.preview} className="dropzone-image">
                                                                            <img
                                                                                alt="Preview"
                                                                                key={file.preview}
                                                                                src={file.preview}
...
export default EditJob = withRouter(EditJob);